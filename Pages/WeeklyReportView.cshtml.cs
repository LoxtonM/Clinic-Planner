...
{
    public class WeeklyReportViewModel : PageModel
    {
        ...
        private readonly IAppointmentDataAsync _appData;
        ...
        public WeeklyReportViewModel(ClinicalContext context, CPXContext cpxContext, IConfiguration config)
        {
            _context = context;
            ...
            _appData = new AppointmentDataAsync(_context);
        ...
        private async Task ClinicListViewSetup(DateTime date, string clinician, string clinic, string searchTerm)
        {
            try
            {
                ...
                var apptList = await _appData.GetAppointments(theDate, theDate.AddDays(7), clinician, clinic);
                ...
                if (clinician != null)
                {
                    ...
                    apptList = apptList.Where(a => a.STAFF_CODE_1 == clinician).ToList();
                ...
                if (clinic != null)
                {
                    ...
                    apptList = apptList.Where(a => a.FACILITY == clinic).ToList();
                }               

                appointmentList = apptList.Where(a => a.BOOKED_DATE >= theDate && a.BOOKED_DATE <= theDate.AddDays(7)).OrderBy(a => a.BOOKED_TIME).OrderBy(a => a.BOOKED_DATE).ToList();
            }
            catch (Exception ex)
            {
                Response.Redirect("Error?sError=" + ex.Message);
